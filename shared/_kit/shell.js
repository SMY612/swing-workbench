  ...
  (function revealOnScroll() {
    ...
    var io = new IntersectionObserver(function (es) {
      ...
      es.forEach(function (e) {
        ...
        // เผยตามเกณฑ์ 0.12 ปกติ · ยกเว้น element ที่สูงเกินจอจน ratio 0.12 เป็นไปไม่ได้
        // (สูง > viewport/0.12 เช่น การ์ดมือถือ card-list ยาว ๆ) — อัตราส่วนแตะ 0.12 ไม่ได้ตลอดกาล
        // → พอโผล่ (ratio ข้าม 0) ก็เผยเลย กันค้าง opacity:0 มองไม่เห็นถาวร (มือถือ workbench)
        var tooTall = e.rootBounds && e.boundingClientRect.height * 0.12 > e.rootBounds.height;
        if (e.intersectionRatio >= 0.12 || tooTall) { e.target.classList.add("in"); io.unobserve(e.target); }
      });
